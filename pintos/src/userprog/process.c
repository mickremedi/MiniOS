...
static void start_process(void *file_name_) {
    ...
    /* Number of bytes needed to allocate data to the stack */
    int num_bytes = strlen(file_name) + 1;
    int word_align = 4 - (num_bytes % 4);

    char *argv_addr[ARG_LIM];
    int argc = 0;

    /* Sets up program arguments */
    char *saveptr;
    char *token = strtok_r(file_name, " ", &saveptr);

    while (token != NULL) {
        int byte_offset = token - file_name;
        argv_addr[argc] = PHYS_BASE - num_bytes + byte_offset;
        argc++;
    ...
    if_.esp -= num_bytes;
    memcpy(if_.esp, file_name, num_bytes);

    /* Word align data */
    if_.esp -= word_align;
    memset(if_.esp, 0, word_align);

    /* Push argv's values onto the stack */
    int argv_size = (argc + 1) * sizeof(char *);
    if_.esp -= argv_size;
    memcpy(if_.esp, argv_addr, argv_size);
